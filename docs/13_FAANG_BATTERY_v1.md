...
| 2026-09-02 | G01 | PASS[P] | ruff 9 -> 0 at 2e395b9: 6x ISC004 parenthesized (no rendered-output change; continuations already correctly spaced), BLE001+S110 noqa with reason at runner.py:115, UP007 PEP604 [MEASURED] |
| 2026-09-02 | G02 | PASS[P] | mypy 7 -> 0, 12 files clean at 2e395b9: INSUFFICIENT_EVIDENCE annotated Literal (killed 3 arg-type at scorer 54-56), isinstance guards at cli.py:106/222 raising TypeError (killed call-overload :111 and arg-type :223 — the one REAL bug), reportlab.* ignore_missing_imports [MEASURED] |
| 2026-09-02 | NOTE | TRY004 | the two guards I added raised ruff's own new finding; RuntimeError -> TypeError. The gate caught it pre-commit — a fix that introduces a lint error is why the gate runs after the patch, not before |
| 2026-09-02 | H03 | FALSE-GREEN CAUGHT | run 33586594787 exited ✓ in 35s having generated ZERO mutants: "Error: No such option: --paths-to-mutate" (mutmut 3.x moved paths to config), swallowed by '|| true'. Artifact was an empty 136-byte XML. Caught by wall-clock implausibility, not by the gate. D0 class in CI form — a measurement step must never carry '|| true'. B6 quote-ban STAYS ARMED [MEASURED: gh run view --log] |
| 2026-09-02 | H03 | ABSTAIN | run 33586917613 FAILED honestly (X, not false-green — the '|| true' removal worked): FileNotFoundError 'config/prompts_v1.json' in test_cli_brand_match.py under mutmut's CWD. Finding: those tests pass only when invoked from repo root; relative config paths assume a working directory. Third environment-assumption defect tonight (/tmp, native-Windows mutmut, CWD). UNBLOCK: make config paths resolve from package root or fixture-inject them, then re-dispatch mutation.yml. B6 quote-ban STAYS ARMED [MEASURED: gh run view --log] |
| 2026-09-02 | RUNNING TOTAL | 31 of 72 | PASS 21 (G01,G02,G12,K07 added) · FAIL 0 · SPLIT/ABSTAIN/PARTIAL 8 (H03 abstain) · POPULATION 72 |
| 2026-09-02 | L01 | PASS[P] | docs/16_USECASE_BATTERY_v1.md: 24 UCs from a 4-recipient x 6-capability matrix, scoped to BUILT capability only; sealed expectation (<8 deliverable) held at 6 of 24; roadmap excluded by construction and enumerated. Supersedes 13's 27-UC seed [MEASURED: file] |
| 2026-09-02 | C08 | PARTIAL | src/aivis/engines.py: OpenAI + Perplexity adapters, additive (runner.py untouched); anthropic delegates to run_once so one impl remains. 15 mocked tests, suite 115->130/3, ruff 0, mypy 0, radon max B(6) [MEASURED]. NOT a C08 pass: no live second-engine row exists yet — blocked on key rotation + OPENAI_API_KEY |
| 2026-09-02 | G10 | NOTE | engines.py adds a SECOND retry loop (_post_with_retries) alongside runner.run_once. Declared in the module docstring, not hidden. Correct end state is one shared helper; requires editing runner.py under a named "go". Sibling of B7's three bundle_digest impls |
