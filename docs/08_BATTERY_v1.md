...
| 2026-09-09 | A08 | AMENDED | Spec: pass = 0 hits outside enumerated classes, classes+counts printed, population = git log -p --all, reference route = grep -a raw bytes. Classes as of today: 03 SE exposure notice x3 (len-84), self-referential spec/ledger rows x3 (len-125/95/95), placeholder setup snippets x2 (sk-ant-... literal, a095b11 study2 docs). Count 3->8 explained: 406d4fc/a095b11 moved state docs into git; every future amendment quoting the patterns adds a self-referential hit. Python route FALSIFIED: cp1252 reader thread died at ~byte 728K and the harness printed "0 total" where the byte-safe route found 8 [MEASURED: 2026-09-09] |
| 2026-09-09 | HARNESS DEFECT | LOGGED | aivis_battery_v2.py:80 subprocess text=True without encoding -> cp1252 on Windows; reader-thread UnicodeDecodeError is BELOW the caller's try/except so the "crashed probe = ABSTAIN" guard never fired; A08 verdict printed on truncated capture. Caught by the traceback in the paste, not by a gate. Fix (encoding utf-8, errors replace) awaiting "go" |
| 2026-09-09 | D04 | PASS | v2 PDF hit was semantic false positive: "one signed client" (contract sense; regex \bsigned\b correct, fired correctly). Resolved by reword at source in NEW emitter study2/make_pdf5.py -> aivis_crediblepr_report_4engine_20260909_v3.pdf; re-grade PASS on shipping bytes, RECONCILE counts identical to v2 run [MEASURED: battery08-d04rerun-20260909.jsonl] |
| 2026-09-09 | HARNESS DEFECT | FIXED | encoding="utf-8", errors="replace" applied at aivis_battery_v2.py:80, operator-executed (execution = go), grep-verified same paste. Supersedes "awaiting go" in the row above and in 02's 2026-09-09 append |
| 2026-09-09 | D04 | SPEC AMENDED | population = aivis-authored report text; verbatim model text inside EXHIBIT BEGIN/END markers excluded (same class as A08's enumerated notices). Reference route: pypdf extract, strip exhibit blocks, grep banned list [MEASURED on v4 bytes: PASS 0 hits] |
| 2026-09-09 | D04 | PASS | v4 pdf sha256 6387d331...b2528, amended population, 0 hits [MEASURED] |
| 2026-09-09 | D05 | PASS | v4 bytes, 0 placeholder chars [MEASURED] |
| 2026-09-09 | NOTE | v4 emitter | make_pdf6.py IMPORTS wilson/present/load + constants from make_pdf5.py (unchanged) - counts cannot drift from v3 (B7 class). New pages: family-x-engine grid, first-three (23-candidate list clears D1 floor), 4 exhibits w/ row hashes. New refusals: sources (16 in-text URLs / 350 clean, computed at emit), sentiment. First-three finding: Claude mention leader (Bospar 17/88) is not its first-three leader (Weber Shandwick, Edelman 7/30 each) [MEASURED] |
| 2026-09-09 | NOTE | CRLF blob defect | git blobs of evidence jsonl were LF-normalized (autocrlf=true, no attributes); diff exactly CRLF-vs-LF. Blob copies of all 4 engine files re-verified: 4x VERDICT PASS, MATCH 88/88+90/90+88/88+84/84, MISMATCH 0 - verifier recomputes from response_text so line endings cannot touch row hashes; byte-identity defect only, evidence intact [MEASURED: /tmp/blobcheck_20260909]. Fix: .gitattributes 4e8867a + renormalize (gated on disk-vs-blob sweep = 0 mismatches). D6/D8 class lesson: backup DONE = blob-hash identity, never push success |
| 2026-09-09 | NOTE | v5 report | cost line removed from Action 1 (pricing-anchor class, operator ruling); OUT -> v5, sha256 473745c5...d0f8d0; graded twice: D04(amended) PASS, D05 PASS, EXHIBIT 4/4, NEW GATE cost-on-face (grep \$\d on aivis-authored text) PASS 0 - added so a marginal-cost figure can never reach a buyer page silently. RECONCILE identical to v4. v4 SUPERSEDED (pricing anchor on face); v5 is the candidate artifact. Eyeball: [operator verdict here] |
| 2026-09-09 | NOTE | v5 eyeball | resolves the unfilled "[operator verdict here]" slot committed at 95fae0a (placeholder-in-ledger, D05 class, logged): operator verdict = VERDICT_WORD on pages 5/7/8 [MEASURED: operator eyeball] |
| 2026-09-09 | v5 eyeball | FAIL | pages 1/5/7 (p8 clean), rasterized inspection: p1 upper-bound line clipped both edges; p5 "most-named X 0/6" alphabetical-tiebreak leak x7 cells; p7 3-of-4 exhibits truncated before captioned firm + unicode-to-"?" mojibake. This row also RESOLVES the two unfilled verdict slots ("[operator verdict here]" @95fae0a; "VERDICT_WORD" uncommitted below-committed rows) as ONE incident, D05 class: NEW RULE - a verdict slot in a paste-ready block will be pasted unfilled; verdicts are typed after looking, never templated [MEASURED: pdftoppm raster] |
| 2026-09-09 | v6 report | PASS + eyeball CLEAN | sha256 686b0074...ec335; fixes F1 line-split, F2 "none of 23 named", F3 excerpt windowed to captioned firm, F4 unicode map, F5 box margin; RECONCILE identical to v4/v5 (third consecutive). 3 NEW GATES all PASS: P1-COMPLETE (bound line ends in full %), P5-NO-ZERO-LEADER, P7-CAPTION-INTEGRITY (every exhibit machine-checked to contain its captioned firm) + P7-NO-MOJIBAKE. Eyeball: pages 1/5/7 re-rasterized CLEAN, p8 clean from v5 pass. v5 SUPERSEDED. v6 is the candidate artifact, R1, DONE all gates [MEASURED] |
| 2026-09-09 | NOTE | exhibits surface canon gap | Perplexity exhibit names Cosmetic PR, Kendrick PR, Illumination PR, Society22 PR - none in canon v1 (Society22 is in 07 teardown competitive set); ChatGPT names DKC, Sunshine Sachs, Trust Relations - also outside. The canon-v2/_v3 rerun argument, made by the evidence on the buyer page [MEASURED: exhibit rows] |
