#!/usr/bin/env python3
"""index_gate.py v0.1 -- CLAIM-GATE-IDX (OMEGA-INDEXFORGE v1.0) as runnable code.
...
Usage:
  python index_gate.py check REC.json [REC.json ...]   # one file per brand/panel row
  python index_gate.py source SCORER.py [SCORER.py ...] # heuristic source known-bads
  python index_gate.py selftest                        # 17 fixtures, EXACT id sets
  python index_gate.py selftest-meta                   # prove the selftest can fail
  python index_gate.py schema                          # print a CLEAN record template

Provenance of the fixtures: every known-bad below is a defect MEASURED on
CAAI v1.0 and recorded in claude/caai-formula-critique-2026-08-19-v1.md
(F1-F8) and claude/caai-recovery-2026-08-19-v1.md. None is invented.
...
NOT COVERED (stated so silence is not read as coverage):
  ...
  - source mode is HEURISTIC pattern-matching on text, not analysis. A clean
    source run is not evidence; a dirty one is.
"""
import sys, os, json, re

VERSION = "index_gate v0.1 / CLAIM-GATE-IDX from OMEGA-INDEXFORGE v1.0"
...
ALL_IDS = [
    ...
    "I4-MISSING-SILENT", "I4-NO-TIER-CAP",
    "I5-NO-INTERVAL",
    "I6-OVERLAP-ORDERED", "I6-UNREPORTED-TIE",
    "I7-NO-MODEL-PIN",
    "I8-CONSTANT-UNCITED", "I8-EST-NO-REVIEW",
    "I9-NO-PRESCRIPTION",
]
...
def run_gate(records, disabled=frozenset()):
    out = []
    for rid, fn in RULES:
        if rid in disabled:
            continue
...
def selftest(disabled=frozenset(), verbose=True):
    ...
    for name, build, expected in cases:
        got = {v["id"] for v in run_gate(build(), disabled)}
        exercised |= got
        if got != expected:
