...
OUT="out/aivis_crediblepr_report_4engine_20260908_v1.pdf"
...
def main():
    ...
    for label,path in ENGINES:
        ...
        E[label]={"rows":len(rows),"clean":clean,"n":len(clean),"err":len(rows)-len(clean),
                  ...
                  "fam":fam,"pres":pres,"overall":overall}
    ...
    y=wrap(c,"WHAT WAS FOUND. %s is named in none of the %d responses, on any engine, in any prompt family, "
        "including every buyer-intent prompt. The same measurement shows which firms are named (page 3), so "
        "the zero is a property of the answers, not of the instrument. The absence holds on four independent "
    ...
    for t,q in [("1. Baseline each client market on these four engines.","Is the client winning where its buyers actually ask? ~$3 per market."),
    ...
    x0=M+150; pw=W-M-x0-24; yb=H-112; bh2=10; gap=4; ggap=16; XMAX=0.40
    c.setFont("Helvetica",7); c.setFillColor(GREY)
    ...
    for l,_ in ENGINES:
        ...
        c.drawString(M,yb,"%s  (n=%d, %s)"%(l,n,e["model"])); yb-=bh2+6
        top3=sorted(e["overall"].items(),key=lambda t:-t[1])[:3]
        for nm,k in top3+[(SUBJ,e["overall"][SUBJ])]:
            lo,hi=wilson(k,n); p=k/n; col=RED if nm==SUBJ else TEAL
            c.setFillColor(BLACK); c.setFont("Helvetica-Bold" if nm==SUBJ else "Helvetica",7.5)
            c.drawRightString(x0-6,yb-bh2+2,"%s %d/%d"%(nm,k,n))
            c.setFillColor(col)
            ...
            yb-=bh2+gap
        yb-=ggap
    wrap(c,"Overall presence per engine: each engine's three most-named firms, then %s in red. Whiskers are 95%% "
        "Wilson intervals. Denominators are clean responses per engine. Where intervals overlap, no separation "
        "is claimed - at these sample sizes most cross-engine gaps do not separate."%SUBJ,M,138,W-2*M,size=8,leading=11,color=GREY)
